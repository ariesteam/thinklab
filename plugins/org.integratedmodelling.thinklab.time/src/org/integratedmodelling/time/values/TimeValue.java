...
public class TimeValue extends ParsedLiteralValue {
	...
	@Override
	public IValue op(String op, IValue ... other)
			throws ThinklabInappropriateOperationException,
			ThinklabValueConversionException {
		IValue ret = null;
		if (op.equals("=")) {
			ret = clone();
			ret.setToCommonConcept(other[0].getConcept(), concept);
			// TODO add sum and subtraction w/ date and period
		} else
			throw new ThinklabInappropriateOperationException(
					"time values do not support operator " + op);
		return ret;
	}
