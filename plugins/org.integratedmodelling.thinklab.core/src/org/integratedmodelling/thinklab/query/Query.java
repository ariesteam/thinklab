...
public class Query implements IQuery, IParseable {
	 ...
	 * @return
	 */
	public Query select(IConcept c) {
		return new Query(c);
	}
	...
	public boolean match(Object i) throws ThinklabException {
		...
		ISemantics semantics = Thinklab.get().conceptualize(i);
		if (semantics == null) {
		...
		if (_subject instanceof IConcept) {
			...
			match = semantics.getConcept().is((IConcept)_subject);
			...
			if (_restrictions == null) {
				...
				for (ISemantics target : semantics.getRelationships((IProperty)_subject)) {
