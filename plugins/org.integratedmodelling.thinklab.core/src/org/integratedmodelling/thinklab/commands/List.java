...
public class List extends CommandPattern {

	enum listmode {DESCRIPTIVE, LIST};
	
	class ListAction implements IAction {
	
		void listOntology(IOntology ont, ICommandOutputReceptor outputWriter) throws ThinklabException {
		
			outputWriter.displayOutput("Listing ontology " + ont);

			outputWriter.displayOutput("\nConcepts:");
			for (IConcept i : ont.getConcepts()) {
				outputWriter.displayOutput("\t" + i.getSemanticType().toString() + ":\t" + i.getLabel() + "\t" + i.getDescription());
			}
			outputWriter.displayOutput("\nProperties:");
		...
		void listIndividual(IInstance c, listmode mode, ICommandOutputReceptor outputWriter) throws ThinklabException {
			...
			if (mode == listmode.DESCRIPTIVE) {
				...
				if (c.getImplementation() == null) {
					...
					outputWriter.displayOutput("has implementation of class " + c.getImplementation().getClass().toString());                
				}
		...
		void listConcept(IConcept c, listmode l, ICommandOutputReceptor outputWriter) throws ThinklabException {
		    ...
		    outputWriter.displayOutput(Polylist.prettyPrint(c.getRestrictions().asList(), 2));
		    
		    outputWriter.displayOutput("  Definition:");
		    outputWriter.displayOutput(Polylist.prettyPrint(c.getDefinition().asList(), 2));
		    
		}
		
		void listProperty(IProperty p, listmode l, ICommandOutputReceptor outputWriter) {
		...
		void listKBox(IKBox kbox, String kbname, ICommandOutputReceptor outputWriter) throws ThinklabException {
		...
		public IValue execute(Command command, ICommandOutputReceptor outputWriter, ISession session, KnowledgeManager km) throws ThinklabException {
			...
			if ("ontologies".equals(subject)) {
				...
				for (IOntology o : km.getKnowledgeRepository().retrieveAllOntologies()) {
					outputWriter.displayOutput(o.getConceptSpace() + ":\t" + o.getURI());
				}
				...
				if (subject.startsWith("#")) {
					...
					IInstance obj = 
						session.retrieveObject(subject.substring(1));
				
					if (obj == null) {
						outputWriter.displayOutput("nothing known about " + subject);
					} else {
					...
					IOntology o = km.getKnowledgeRepository().retrieveOntology(subject);
					if (o != null)
	...
	public CommandDeclaration createCommand() {
		...
		CommandDeclaration ret = new CommandDeclaration("list", "list resources and their contents");
		
		try {
			ret.addOptionalArgument("subject", "what to list", 
					KnowledgeManager.get().getTextType().getSemanticType(), "__NONE__");
			ret.addOption("l", "lf", "list format", "", null);
