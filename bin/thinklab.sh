...
if [ -z "$THINKLAB_MEMORY" ] ; then
  THINKLAB_MEMORY=12000
fi

if [ -z "$THINKLAB_PERMSIZE" ] ; then
  THINKLAB_PERMSIZE=1024
fi
...
JAVA_OPTS="-server -XX:MaxPermSize=${THINKLAB_PERMSIZE}M -Xmx${THINKLAB_MEMORY}M -Xms${THINKLAB_MEMORY}M -Djava.awt.headless=true"
THINKLAB_CMD="$JAVACMD $JAVA_OPTS -Djava.library.path=$THINKLAB_INST/plugins/org.integratedmodelling.thinklab.riskwiz/common -Djpf.boot.config=$THINKLAB_INST/boot.properties -Dthinklab.plugins=$THINKLAB_PLUGINS -Dthinklab.inst=$THINKLAB_INST -jar $THINKLAB_INST/lib/im-boot.jar org.java.plugin.boot.Boot"
...
cd $CWD
>>>>>>> branch 'refs/heads/dev' of https://fvilla@github.com/ariesteam/thinklab.git
